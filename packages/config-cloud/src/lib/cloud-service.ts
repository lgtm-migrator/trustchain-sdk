...
import { Config } from '@trustcerts/config';
import { DecryptedKeyPair, SignatureType } from '@trustcerts/crypto';
...
export class CloudService {
  ...
  constructor(private url: string, private persistName = 'login') {
    ...
    this.authApi = new AuthorizeApi(configuration);
  ...
  public async getConfig(): Promise<Config> {
    ...
    const keyPair = await this.decryptWallet(configResponse.data.wallet).catch(
      () => []
    );
    return {
      keyPairs: keyPair,
  ...
  private getAuthApiWithCredentials() {
    ...
    return new AuthorizeApi(configuration);
  }
}
